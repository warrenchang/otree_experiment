...
class Subsession(BaseSubsession):
    ...
    def before_session_starts(self):
        ...
        for p in self.get_players(): # set interaction number and round number
            ...
            p.compete = Constants.competes[interaction_number-1]
        ...
        if round_in_interaction == 1:
            paying_round = random.randint(1, Constants.interaction_length[interaction_number-1])
        ...
        print('Session paying round',self.session.vars['paying_round'] )
...
class Group(BaseGroup):
    def interact(self):
        ...
        if p1.round_in_interaction == self.session.vars['paying_round']:
